...
MEAN_LAG_TIME           = 804.8 # [min]  # Happens in the beginning of each cycle. Cells will not further its cell cycle while in lag.
MEAN_CELL_CYCLE_TIME    = 162.3 # 90 # [min]
EXPERIMENT_TIME         = 1 #72*60 # [min]


# vectors
LAG_TIMES               = MEAN_LAG_TIME * np.ones([FOUNDER_COUNT]) #+ #np.random.normal(0, 1, [FOUNDER_COUNT])
CELL_CYCLE_TIMES        = MEAN_CELL_CYCLE_TIME * np.ones([FOUNDER_COUNT]) #+ np.random.normal(0, 1, [FOUNDER_COUNT])
